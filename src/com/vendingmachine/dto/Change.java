...
public class Change {
	...
	public static BigDecimal changeDueInPennies(BigDecimal itemCost, BigDecimal money) {
		...
		System.out.println("Change due: $"
				+ (changeDueInPennies.divide(new BigDecimal("100"), 2, RoundingMode.HALF_UP).toString()));
		...
		for (BigDecimal coin : coins) {
			...
			if (changeDueInPennies.compareTo(coin) >= 0) {
				...
				if (!changeDueInPennies.remainder(coin).equals(zero)) {
					// the number of coins of coin[i] required will be the floor division of change
					// amount/coin
