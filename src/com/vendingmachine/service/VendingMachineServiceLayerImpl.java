...
public class VendingMachineServiceLayerImpl implements VendingMachineServiceLayer {
	...
	public void checkIfEnoughMoney(Item item, BigDecimal inputMoney) throws InsufficientFundsException {
		...
		if (item.getCost().compareTo(inputMoney) == 1) {
			throw new InsufficientFundsException("ERROR: insufficient funds, you have only input $" + inputMoney);
		}
	}
	...
	public Item getItem(String name, BigDecimal inputMoney)
		...
		if (wantedItem == null) {
			throw new NoItemInventoryException("ERROR: " + name + " is not available in the vending machine.");
		}

		// Check if the user has input enough money
		checkIfEnoughMoney(wantedItem, inputMoney);

		// If they have, check that the item is in stock and if so, remove one item from
		// the inventory
...
//        return getChangePerCoin(wantedItem,inputMoney);
	...
	public void removeOneItemFromInventory(String name)
			throws NoItemInventoryException, VendingMachinePersistenceException {
		// Remove one item from the inventory only when there are items to be removed,
		// i.e. inventory>0.
